...
@Injectable()
export class SuiteRepository implements IBaseRepository<
  Suite,
  Prisma.SuiteCreateInput,
  Prisma.SuiteUpdateInput
> {
  ...
  async reconcileAllSuiteStatuses(): Promise<void> {
    ...
    await Promise.all(suites.map((s) => this.reconcileSuiteStatus(s.id)));
  }
